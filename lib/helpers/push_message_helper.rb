module PushMessageHelper
  ...
  #app_inner_url跳转
  def self.notify_app_inner_url_options url, title, message, tag=nil
    base_options "app_inner_url", nil, nil, nil, url, title, message, tag
  end
  ...
  # title  提示框的标题
  # message  提示框的描述信息
  def self.base_options action_name, type, page_title, uuid, url, title, message, tag=nil
    {
      tag: tag,#for statistic
      action: action_name,
      params: {
        type: type,
        uuid: uuid,
